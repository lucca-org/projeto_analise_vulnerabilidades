...
if ! apt-key list 2>/dev/null | grep -q "$KALI_KEY"; then
    echo "Kali archive key not found. Installing..."
    ...
    if command -v wget >/dev/null 2>&1; then
        ...
        if command -v gpg >/dev/null 2>&1; then
            ...
            gpg --export "$KALI_KEY" | apt-key add -
        else
            echo -e "${RED}No tools available to fetch the key.${NC}"
            echo "Please install either wget, curl, or gpg."
            exit 1
        fi
    fi
    
    # Try to install the keyring package
    echo "Attempting to install kali-archive-keyring package..."
    apt-get update --allow-insecure-repositories --allow-unauthenticated || true
    apt-get install -y kali-archive-keyring --allow-unauthenticated || {
        echo -e "${YELLOW}Could not install kali-archive-keyring package. Key may still be imported.${NC}"
    }
    
    echo -e "${GREEN}✓ Kali archive key setup completed${NC}"
else
    echo -e "${GREEN}✓ Kali archive key already present${NC}"
fi
...
for mirror in "${MIRRORS[@]}"; do
    ...
    if timeout 5 wget -q --spider "$mirror/dists/kali-rolling/Release"; then
        ...
        echo "deb $mirror kali-rolling main contrib non-free" >> /etc/apt/sources.list
